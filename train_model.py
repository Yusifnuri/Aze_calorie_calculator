...
def compute_class_weights(dataset):
    ...
    # Extract all labels as Python integers
    labels = []
    ...
    for i in sorted_indices[:5]:
        i_val = int(i.item())  # Convert to Python int
        class_name = dataset.classes[i_val]
        weight = weights_tensor[i_val].item()
        count = int(np.sum(labels == i_val))
