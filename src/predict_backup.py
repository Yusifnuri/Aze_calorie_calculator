...
SECONDARY_CONFIDENCE_THRESHOLD = 0.20  # Increased from 0.15 to 0.20
...
def apply_post_processing_filter(detected_dishes, top_confidence):
    ...
    filtered = []
    CONFIDENCE_GAP_THRESHOLD = 0.60  # Secondary dish must be at least 60% of primary
    ...
    for i, dish_info in enumerate(detected_dishes):
        if i == 0:  # Always keep primary dish
            ...
            # Check if confidence gap is reasonable
            if confidence_ratio >= CONFIDENCE_GAP_THRESHOLD:
                filtered.append(dish_info)
